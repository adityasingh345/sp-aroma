...
const ProductDetailPage = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const mappedAll = items.map((it: any) => ({
          ...
          type: (it.status === 'attar' || it.product_name?.toLowerCase().includes('attar')) ? 'Attar' : 'Perfume',
  ...
  const accordionItems = [
    { title: 'Description', content: product.longDescription || 'No description available.' },
    { title: 'Ingredients', content: product.ingredients || 'Ingredients information not available.' },
    { title: 'How to Use', content: product.howToUse || 'Usage instructions not available.' },
  ];
